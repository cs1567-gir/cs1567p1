...
def odometry_callback(data):
    ...
    #print "angle: ", math.degrees(robot_state.heading)
...
def reset_odom(parameters):
    pub = rospy.Publisher('/mobile_base/commands/reset_odometry', Empty, queue_size=10)
    while pub.get_num_connections() < 1:
        rospy.sleep(0.1)
    pub.publish(Empty())
    robot_state.reset_class()
...
def move_service(parameters):
    if parameters.type == 'move_to':
        ...
        retval = robot_state.move_distance(parameters.a, parameters.b)
...
def initialize():
    ...
    pub = rospy.Publisher('/mobile_base/commands/reset_odometry', Empty, queue_size=10)
    rospy.Subscriber('/mobile_base/sensors/core', SensorState, core_callback)
    ...
    s5 = rospy.Service('cs1567_reset_odometry', CSReset, reset_odom)
